module zm_conv_common
  ...
  use error_function,  only: erfc ! mathematical error function under /models/atm/cam/src/utils/
  ...
  save
    ...
    !zmh - Namelist variables
    !===============================================================================
    logical           :: zmh_shallow_lf         = .true. !false. !true.  !include large-scale forced
    logical           :: zmh_shallow_ent        = .false. !false. !true.  !new entrainment
    real(r8)          :: zmh_ent0               =  0.015
    ...
    ! WL** hard coded for now - come from namelist in original code **WL
    real(r8)          :: zmh_ufrcd              =  2.0    !coef of detrained volume
    real(r8)          :: zmh_ufrc0              =  2.0    !coef
    real(r8)          :: zmh_ufrcz              =  2.0    !offset z^2 coef
    character(len=16) :: shallow_closure_scheme = 'cam' !prognostic' !'cam'
    real(r8)          :: zmh_shallow_tau1       = 600.  ! for prognostic cape , time to mb
    real(r8)          :: zmh_shallow_tau2       = 600.  !
    real(r8)          :: zmh_rkm                    = 10.0
    real(r8)          :: uwshcu_rpen = 15.0

    real(r8)          :: zmh_shallow_xyt        =  0.0
    real(r8)          :: zmh_shallow_xyq        =  0.0
    real(r8)          :: zmh_shallow_tpertmax   = 2.0
    real(r8)          :: zmh_shallow_qpertmax   = 1.0e-3
    real(r8)          :: zmh_shallow_fwcrit     = 1.0

    logical           :: zmh_shallow_pertt      = .false.
    logical           :: zmh_shallow_pertw      = .false.
    logical           :: zmh_shallow_qpertf     = .false.
    logical           :: zmh_shallow_tpertf     = .false.
    logical           :: zmh_shallow_tpertb     = .false.
    logical           :: zmh_shallow_qperth     = .false.
...
module zwshcu
  ...
  public :: zwshcu_init, zwshcu_run, zwshcu_finalize
...
!!
  subroutine zwshcu_init(latvap, latice_in, cpair, rair, ep1, ep2, gravit, hypi, rocp, errmsg, errflg)

    implicit none

    real(kind=kind_r8), intent(in) :: latvap, latice_in, cpair, rair, ep1, ep2, gravit, hypi, rocp
    ...
    ! Initialization of ZW shallow cumulus scheme constants
    xlv = latvap !  Latent heat of vaporization
    xlf = latice_in !  Latent heat of fusion
    xls = xlv + xlf !  Latent heat of sublimation = xlv + xlf
    cp = cpair      !  Specific heat of dry air
    r = rair        ! Gas constant for dry air
    g = gravit      ! Gravitational constant
    zvir = ep1      ! rh2o/rair - 1 (ratio of vapor to dry air gas constants minus one)
    p00 = hypi      ! Reference pressure for exner function
    rovcp = rocp    ! rd/cp

  end subroutine zwshcu_init

!-----------------------------------------------------------------------
!
! Purpose:
! Main driver for ZW shallow convection scheme
!
! Method: ZW is Modified UW shallow cumulus scheme by Sungsu Park and Christopher S. Bretherton!
!
...
!-----------------------------------------------------------------------
!
!> \section arg_table_zwshcu_run Argument Table
!! \htmlinclude zwshcu_run.html
!!
  subroutine zwshcu_run(mix, mkx, iend, ncnst, dt, lat, lon, coszrs,                                &
                        ps0, zs0, p0, z0, dp0, u0, v0, qv0, ql0, qi0, t0, s0, tr0,                  &
                        tke, cldfrct, concldfrct, pblh, cush, umf, slflx, qtflx, flxprc1, flxsnow1, &
                        qvten, qlten, qiten, sten, uten, vten, trten, qrten, qsten,                 &
                        precip, snow, evapc, cufrac, qcu, qlu, qiu,                                 &
                        cbmf, qc, rliq, omega, massflx_sh, tpert, qpert, gradt, gradq, landfrac,    &
                        cnt, cnb, qsat, lchnk, wu, errmsg, errflg)

    !## In    ##
    integer , intent(in)    :: mix      ! horizontal loop extent
    integer , intent(in)    :: mkx      ! vertical_dimension (top layer from the surface)
    integer , intent(in)    :: iend     ! horizontal_loop_extent_for_block
    integer , intent(in)    :: ncnst    ! number of tracers
    integer , intent(in)    :: lchnk    ! ccpp_block_number

    real(r8), intent(in)    :: dt       ! physics time step = 2 * delta_t [model time increment] [s]

    real(r8), intent(in)    :: ps0(mix,0:mkx)       !  Environmental pressure at the interfaces [ Pa ]
    real(r8), intent(in)    :: zs0(mix,mkx+1)       !  Environmental height at the interfaces   [ m ]
    real(r8), intent(in)    :: p0(mix,mkx)          !  Environmental pressure at the layer mid-point [ Pa ]
    real(r8), intent(in)    :: z0(mix,mkx)          !  Environmental height at the layer mid-point [ m ]
    real(r8), intent(in)    :: dp0(mix,mkx)         !  Environmental layer pressure thickness [ Pa ] > 0.
    real(r8), intent(in)    :: u0(mix,mkx)          !  Environmental zonal wind [ m/s ]
    real(r8), intent(in)    :: v0(mix,mkx)          !  Environmental meridional wind [ m/s ]
    real(r8), intent(in)    :: qv0(mix,mkx)         !  Environmental water vapor specific humidity [ kg/kg ]
    real(r8), intent(in)    :: ql0(mix,mkx)         !  Environmental liquid water specific humidity [ kg/kg ]
    real(r8), intent(in)    :: qi0(mix,mkx)         !  Environmental ice specific humidity [ kg/kg ]
    real(r8), intent(in)    :: t0(mix,mkx)          !  Environmental temperature [ K ]
    real(r8), intent(in)    :: s0(mix,mkx)          !  Environmental dry static energy [ J/kg ]
    real(r8), intent(in)    :: tr0(mix,mkx,ncnst)   !  Environmental tracers [ #, kg/kg ]
    real(r8), intent(in)    :: tke(mix,mkx+1)       !  Turbulent kinetic energy at the interfaces [ m2/s2 ] *** add in host side **
    real(r8), intent(in)    :: cldfrct(mix,mkx)     !  Total cloud fraction at the previous time step [ fraction ]
    real(r8), intent(in)    :: concldfrct(mix,mkx)  !  Total convective ( shallow + deep ) cloud fraction at the previous time step [ fraction ] *** add in host side **
    real(r8), intent(in)    :: pblh(mix)            !  Height of PBL [ m ]
    real(r8), intent(in)    :: lat(mix)             !  Latitudes in radian
    real(r8), intent(in)    :: lon(mix)             !  Longitudes in radian
    real(r8)                :: coszrs(mix)          !  Cosine solar zenith angle
    ! ZMH
    real(r8), intent(in)    :: omega(mix,mkx)       !  Vertical velocity [Pa s-1]
    real(r8), intent(in)    :: massflx_sh(mix)      !  Cumulus_cloud base mass flux at pervious timestep ??? [ kg m-2 s-1 ]
    real(r8), intent(in)    :: tpert(mix)           !  Pertubation of temperature at the surface [ K ]
    real(r8), intent(in)    :: qpert(mix)           !  Pertubation of water vapor mixing ratio at the surface [ kg/kg ]
    real(r8), intent(in)    :: gradt(mix)           !  Temperature gradient due to frontogenesis???
    real(r8), intent(in)    :: gradq(mix)           !  Moisture gradient due to frontogenesis???
    real(r8), intent(in)    :: landfrac(mix)        !  Land area fraction
    real(r8), intent(in)    :: dpdry0(mix,mkx)      !  Environmental dry layer pressure thickness [ Pa ]

    integer , external      :: qsat

    !## InOut ##
    real(r8), intent(inout) :: cush(mix)            !  Convective scale height [ m ] *** add in host side **

    !##   Out ##
    real(r8), intent(  out) :: umf(mix,mkx+1)           !  Updraft mass flux at the interfaces [ kg/m2/s ]
    real(r8), intent(  out) :: slflx(mix,mkx+1)         !  Updraft liquid static energy flux [ J/kg * kg/m2/s ]
    real(r8), intent(  out) :: qtflx(mix,mkx+1)         !  Updraft total water flux [ kg/kg * kg/m2/s ]
    real(r8), intent(  out) :: flxprc1(mix,mkx+1)       !  uw grid-box mean rain+snow flux (kg m^-2 s^-1) for physics buffer calls in convect_shallow.F90
    real(r8), intent(  out) :: flxsnow1(mix,mkx+1)      !  uw grid-box mean snow flux (kg m^-2 s^-1) for physics buffer calls in convect_shallow.F90
    real(r8), intent(  out) :: qvten(mix,mkx)           !  Tendency of water vapor specific humidity [ kg/kg/s ]
    real(r8), intent(  out) :: qlten(mix,mkx)           !  Tendency of liquid water specific humidity [ kg/kg/s ]
    real(r8), intent(  out) :: qiten(mix,mkx)           !  Tendency of ice specific humidity [ kg/kg/s ]
    real(r8), intent(  out) :: sten(mix,mkx)            !  Tendency of dry static energy [ J/kg/s ]
    real(r8), intent(  out) :: uten(mix,mkx)            !  Tendency of zonal wind [ m/s2 ]
    real(r8), intent(  out) :: vten(mix,mkx)            !  Tendency of meridional wind [ m/s2 ]
    real(r8), intent(  out) :: trten(mix,mkx,ncnst)     !  Tendency of tracers [ #/s, kg/kg/s ]
    real(r8), intent(  out) :: qrten(mix,mkx)           !  Tendency of rain water specific humidity [ kg/kg/s ]
    real(r8), intent(  out) :: qsten(mix,mkx)           !  Tendency of snow specific humidity [ kg/kg/s ]
    ...
    real(r8), intent(  out) :: evapc(mix,mkx)           !  Tendency of evaporation of precipitation [ kg/kg/s ]
    real(r8), intent(  out) :: cufrc(mix,mkx)           !  Shallow cumulus cloud fraction at the layer mid-point [ fraction ]
    real(r8), intent(  out) :: qcu(mix,mkx)             !  Liquid+ice specific humidity within cumulus updraft [ kg/kg ]
    real(r8), intent(  out) :: qlu(mix,mkx)             !  Liquid water specific humidity within cumulus updraft [ kg/kg ]
    real(r8), intent(  out) :: qiu(mix,mkx)             !  Ice specific humidity within cumulus updraft [ kg/kg ]
    real(r8), intent(  out) :: cbmf(mix)                !  Cumulus cloud base mass flux [ kg/m2/s ]
    real(r8), intent(  out) :: qc(mix,mkx)              !  Tendency of cumulus condensate detrained into the environment [ kg/kg/s ]
    real(r8), intent(  out) :: rliq(mix)                !  Vertical integral of tendency of detrained cloud condensate qc [ m/s ]
    !ZMH
    real(r8), intent(  out) :: cnt(mix)                 !  Cumulus top  interface index [], cnt = kpen where kpen is the highest layer with positive updraft vertical velocity - top layer cumulus can reach
    real(r8), intent(  out) :: cnb(mix)                 !  Cumulus base interface index [], cnb = krel - 1 where krel is the release layer where buoyancy sorting mixing occurs for the first time
    real(r8), intent(  out) :: wu_inv(mix,mkx+1)        !  Convective updraft vertical velocity [ m/s]
    ...
    ! q front
    if(zmh_shallow_qpertf)then
        qpert(:) = qpert(:) + gradq(:)*zmh_shallow_xyq*cos(lat(:))*max(coszrs(:),0.0)* &
                             (200.0/max(sgh30(:,lchnk),200.))
    endif
    ! q terrain
    if(zmh_shallow_qperth)then
        do i= 1,mix
        qpert(i) = qpert(i) + abs(qv0(i,3) - qv0(i,1))/(z0(i,3) - z0(i,1))* sgh30(i,lchnk)*0.5
        enddo
    endif

    ! t front
    if(zmh_shallow_tpertf)then
        tpert(:) = tpert(:) + gradt(:)*zmh_shallow_xyt*cos(lat(:))*max(coszrs(:),0.0)* &
                             (200.0/max(sgh30(:,lchnk),200.))
    endif
    ! t breeze
    if(zmh_shallow_tpertb)then
        do i=1,mix
        if(abs(landfrac(i)-0.5)  .le.  0.45 .or. sgh30(i,lchnk) .gt. 50.)then
            tpert(i) = tpert(i) + gradt(i)*zmh_shallow_xyt*max(coszrs(i),0.)*cos(lat(i))
!        qpert(i) = qpert(i) + gradq(i)*zmh_shallow_xyq*max(coszrs(i),0.)*cos(lat(i))
        endif
        enddo
    endif

    !print*,'zwtpert(:)',tpert
    !print*,'zwqpert(:)',qpert*1.0e3


    do i=1,mix
        tpert(i) = min(tpert(i), zmh_shallow_tpertmax)
        qpert(i) = min(qpert(i), zmh_shallow_qpertmax)
    enddo

    !#endif

    ! ----------------------
    if(zmh_shallow_pertt)then
        do i=1,iend
        do k=1,mkx
        if(zs0(i,k-1) .gt. pblh(i))cycle
        qv0(i,k)       = qv0(i,k) + qpert(i)
        t0(i,k)        = t0(i,k)  + tpert(i)
        s0(i,k)        = s0(i,k)  + cp*tpert(i)
        enddo
        enddo
    endif
    !zmh end
    ...
    call compute_zwshcu(mix  , mkx    , iend      , ncnst , dt   , &
                        ...
                        cbmf , qc     , rliq      ,                &
                        !zmh
                        omega, massflx_sh    , tpert, &
                        cnt  , cnb    , qsat      , lchnk , dpdry0 ,wu)  !wxc



  end subroutine zwshcu_run

  subroutine zwshcu_finalize()
  end subroutine zwshcu_finalize

! CCPP-compliant code ends here

! ******* original subroutine of University of Washington Shallow Convection Scheme ******
...
!zmh for prognostic
        etau1_zmh =  1._r8 - exp(- dt/2._r8/zmh_shallow_tau1)  
        etau2_zmh =  exp(- dt/2._r8/zmh_shallow_tau2)  
        random_zmh(:) = 0.2_r8
        rkm = zmh_rkm
...
!zmh ==============
   if(zmh_shallow_lf)then
           tmp1 = min(max(-omega(i,kinv-1),0._r8),3.0)
           wcrit = wcrit-tmp1 /rho0inv/9.8_r8
           wcrit = max(wcrit, 0._r8) 
   endif   
!zmh ==============

   wcrit  = wcrit * zmh_shallow_fwcrit       !tunable parameter
! ================================
...
!zmh ==============
   ...
   if(zmh_shallow_stochastic .and. k<0)then  !! stochatic not tested yet
...
!zmh ==============
   if(zmh_shallow_stochastic .and. k<0)then  !stochastic disabled for now
...
!                fer = min( max(fer, 0.0_r8), 3.0e-2_r8)
                ...
                ! xwang
                bogbot = rbuoy * ( thvu(km1) / thvebot  - 1._r8 ) * g
                fer(k) = zmh_ent0 * bogbot/max(wue*wue,1.0e-4_r8)   !0.25 wxc
                fer_min = 0._r8
                if(bogbot .gt. 1.e-4_r8 ) fer_min = 1.e-4_r8
                fer(k) = min( max(fer(k), fer_min), 3.0e-2_r8)

!zmh            !    fdr(k) = -(-137.)*(-bogbot)**2 - (-3.5+1)*(-bogbot) - (-0.012)
!zmh                fdr(k) = 137.*(bogbot/wue**2)**2 - 2.5*bogbot/wue**2 + 0.012
!zmh                fdr_min = 0._r8
!zmh                if(bogbot .lt. 1.e-4_r8 ) fdr_min = 1.e-4_r8
!zmh                fdr(k) = min( max(fdr(k), fdr_min), 3.0e-2_r8)
...
!zmh============================
     ...
     if(zmh_cloud_sh)then
       ...
       tmp1 = (z0(k)-z0(krel))/( z0(kpen)-z0(krel) +10._r8)
!       tmp1 = 1.0 - zmh_ramp(tmp1, 0._r8, 1.0_r8)
       tmp1 = 1.0 + zmh_ufrcz * tmp1
